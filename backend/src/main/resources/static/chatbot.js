...
(function () {
  ...
  /* ── Extract source city from messages like "Delhi to Goa" / "from Mumbai" ── */
  function detectSource(msg) {
    const fromMatch = msg.match(/from\s+([a-z]+)/i);
    if (fromMatch) return fromMatch[1];
    const toMatch = msg.match(/([a-z]+)\s+to\s+/i);
  ...
  async function respond(input) {
    ...
    const detectedSource  = detectSource(msg);
    ...
    if (detectedSource) memory.source   = detectedSource;
    ...
    if (msg.includes('hotel') || msg.includes('stay') || msg.includes('accommodation') || msg.includes('hostel') || msg.includes('resort') || msg.includes('where to stay')) {
      if (memory.city) return await hotelResp(memory.city, memory.budget);
    ...
    if (msg.includes('flight') || msg.includes('train') || msg.includes('bus') || msg.includes('transport') || msg.includes('how to reach') || msg.includes('how to go') || msg.includes('reach') || msg.includes('travel to') || msg.includes('get to')) {
      if (memory.city) return await transportResp(memory.city, memory.source || detectedSource);
  ...
  /* ── Hotel response ── */
  async function hotelResp(city, budget) {
    const d = DEST[city];
    try {
      const cityName = d ? d.name : city;
      const url = budget
        ? `${YAMI_API}/hotels/search?city=${encodeURIComponent(cityName)}&maxPrice=${budget}`
        : `${YAMI_API}/hotels/search?city=${encodeURIComponent(cityName)}`;
      const res = await fetch(url);
      const hotels = await res.json();
      if (Array.isArray(hotels) && hotels.length) {
        const sorted = hotels.slice().sort((a, b) => a.pricePerNight - b.pricePerNight);
        const lines = sorted.slice(0, 4).map(h =>
          `🏨 **${h.hotelName}** — ₹${Number(h.pricePerNight).toLocaleString('en-IN')}/night · ⭐${h.rating} · ${h.availableRooms} rooms left\n   📍 ${h.address}`
        ).join('\n\n');
        return `Here are the best hotels in **${cityName}** ${d?.emoji || ''}:\n\n${lines}\n\nHead to **Plan a Trip** to book one${memory.name ? ', ' + memory.name : ''}! 🎯`;
      }
    ...
    const h = hotels[city];
    if (!h) return `I can find great hotels in ${d?.name || city}! What's your budget per night? 🏨`;
    if (budget && budget < 5000)  return `For **${d.name}** on a budget:\n\n💚 **${h.low}**\n\nGreat value and you'll meet amazing fellow travellers${memory.name ? ', ' + memory.name : ''}!`;
    if (budget && budget < 15000) return `For **${d.name}** mid-range:\n\n💛 **${h.mid}**\n\nComfort without overspending${memory.name ? ', ' + memory.name : ''}!`;
    if (budget && budget >= 15000) return `${memory.name ? memory.name + ', you' : 'You'} deserve the best! For **${d.name}**:\n\n👑 **${h.high}**\n\nAbsolutely world-class. Unforgettable! 🌟`;
    return `For **${d.name}** ${d.emoji}:\n\n💚 **Budget:** ${h.low}\n💛 **Mid-range:** ${h.mid}\n👑 **Luxury:** ${h.high}\n\nWhat's your budget per night${memory.name ? ', ' + memory.name : ''}?`;
  }

  /* ── Transport response ── */
  async function transportResp(city, sourceCity) {
    const d = DEST[city];
    const cityName = d ? d.name : city;
    if (sourceCity) {
      try {
        const res = await fetch(`${YAMI_API}/transport/search?source=${encodeURIComponent(sourceCity)}&destination=${encodeURIComponent(cityName)}`);
        const options = await res.json();
        if (Array.isArray(options) && options.length) {
          const icons = { Flight: '✈️', Train: '🚂', Bus: '🚌' };
          const lines = options.slice(0, 4).map(t =>
            `${icons[t.type] || '🚀'} **${t.type}** ${t.source} → ${t.destination}  -  ₹${Number(t.fare).toLocaleString('en-IN')} · ${t.departureTime || ''} – ${t.arrivalTime || ''} · ${t.availableSeats} seats`
          ).join('\n');
          return `Getting from **${sourceCity}** to **${cityName}** ${d?.emoji || ''}:\n\n${lines}\n\nBook in the **Trip Planner**${memory.name ? ', ' + memory.name : ''}! 🎯`;
        }
      } catch { /* fall through */ }
    }
    ...
    // Fallback hardcoded
    const routes = {
      goa:    '✈️ **Flight Delhi→Goa**  -  IndiGo/SpiceJet from ₹3,200 (~2.5 hrs)\n✈️ **Flight Mumbai→Goa**  -  from ₹2,800 (~1 hr)\n🚂 **Train Mumbai→Goa**  -  from ₹900 (overnight, scenic)\n🚌 **Bus Bangalore→Goa**  -  ₹700 (overnight Volvo)',
      manali: '✈️ **Flight Delhi→Kullu**  -  from ₹5,500 (45 min, then taxi)\n🚌 **HRTC Volvo Delhi→Manali**  -  ₹850 (~14 hrs overnight)\n🚌 **Bus Chandigarh→Manali**  -  ₹600 (7 hrs)',
      jaipur: '🚂 **Train Delhi→Jaipur**  -  Shatabdi ₹250 (4.5 hrs)\n✈️ **Flight Bangalore→Jaipur**  -  from ₹4,200 (2 hrs)\n🚌 **Bus Delhi→Jaipur**  -  RSRTC Volvo ₹450 (5 hrs)',
      ...
      ladakh: '✈️ **Flight Delhi→Leh**  -  IndiGo from ₹7,500 (1.5 hrs)\n🚌 **Manali→Leh Highway Bus**  -  ₹550 (15–16 hrs, legendary road!)\n🚌 **Srinagar→Leh**  -  JKSRTC ₹480',
    };
    const route = routes[city];
    if (!route) return `Tell me your starting city too${memory.name ? ', ' + memory.name : ''} (e.g. *"Delhi to ${cityName}"*) and I'll find transport options! 🚀`;
    return `Getting to **${cityName}** ${d?.emoji || ''}:\n\n${route}\n\nFor live options, try the **Trip Planner**${memory.name ? ', ' + memory.name : ''}! 😊`;
  }
  ...
  window.yamiSend = async function () {
    ...
    const reply = await respond(txt);
    stopTyping();
    botMsg(reply);
  };

  window.yamiQ = async function (txt) {
    userMsg(txt);
    typing();
